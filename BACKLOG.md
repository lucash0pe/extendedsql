...
- [ ] **Nested / multi-grain aggregation (headline feature — the grain-bridging companion to
  ...
  `public/docs/syntax.md` section + an ESQL demo example. Pairs with CONTAINS: CONTAINS
...
The demo frontend lives in `portfolio/` (`src/demos/esql/`) and its data in the `datasets` repo
(`datasets/<name>/esql/demo.py`, which calls `esql.demokit.build_demo`). This repo is engine-only,
with `sales.csv` as its sole fixture. See `portfolio` and `datasets` for demo work.
